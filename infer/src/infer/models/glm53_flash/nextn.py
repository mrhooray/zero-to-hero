...
_NEXTN_PREFIX = checkpoint.DENSE_LAYER_PREFIX.format(
    layer_id=glm53_flash.NEXTN_LAYER_ID
)
...
def _nextn_checkpoint_specs() -> dict[str, checkpoint.WeightSpec]:
    direct = (
        checkpoint.WeightSpec(
            "eh_proj.weight", (4096, 8192), "BF16", None, _NEXTN_PREFIX
        ),
...
def _allocate_nextn_prefix_snapshots(
    ...
    return GLM53NextNPrefixSnapshots(
        ...
        draft_hidden=empty(
            (snapshot_slot_count, glm53_flash.HIDDEN_SIZE), torch.bfloat16
        ),
    )

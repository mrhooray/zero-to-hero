...
GLM53_TARGET_KDA_LAYER_IDS = (
    *glm53_flash.DENSE_LAYER_IDS,
    *glm53_flash.SPARSE_KDA_LAYER_IDS,
)
...
def load_glm53_target_weights(
    ...
    if not 0 <= tp_rank < glm53_flash.TP_SIZE:
        raise ValueError(
            f"tp_rank must be in [0, {glm53_flash.TP_SIZE}), got {tp_rank}"
        )
...
def _allocate_target_prefix_snapshots(
    ...
    sparse_mla_layers = tuple(
        glm53_flash.allocate_sparse_mla_prefix_snapshots(
...
def _target_decode_workspace_view(
...
def _allocate_sparse_mla_projection_workspace(
    ...
    zeros = lambda shape, dtype: torch.zeros(shape, dtype=dtype, device=device)
    shapes = glm53_flash.sparse_mla_projection_workspace_shapes(
...
def _allocate_sparse_mla_decode_workspace(
    ...
    zeros = lambda shape, dtype: torch.zeros(shape, dtype=dtype, device=device)
    shapes = glm53_flash.sparse_mla_decode_workspace_shapes(
        batch_size, attention_tp_size
    )
...
def _allocate_sparse_mla_output_workspace(
    ...
    zeros = lambda shape, dtype: torch.zeros(shape, dtype=dtype, device=device)
    shapes = glm53_flash.sparse_mla_output_workspace_shapes(
        batch_size, attention_tp_size
    )
...
def _allocate_prefill_staging(
    ...
    return _GLM53TargetPrefillStaging(
        ...
        block_table=gpu(
            (capacity, glm53_flash.SPARSE_MLA_COMPOUND_BLOCKS), torch.int32
        ),
...
def _validate_target_weights(weights: GLM53TargetWeights[object]) -> None:
    _require_count(
        "dense weights", weights.dense_layers, len(glm53_flash.DENSE_LAYER_IDS)
    )
    _require_count(
        "sparse KDA weights",

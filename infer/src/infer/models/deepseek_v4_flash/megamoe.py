...
def moe_workspace_shapes(
    ...
    return _moe_workspace_shapes(
        token_count, deepseek_v4_flash.SHARED_INTERMEDIATE_SIZE
    )
...
def _pack_moe_weights(
    ...
    *,
    ...
    def take(
        spec: deepseek_v4_flash.WeightSpec, expert_id: int | None = None
    ) -> object:
...
def _validate_hash_routes_tensor(routes: object, torch: object) -> None:
    ...
    if bool(
        (
            (ordered[:, 0] < 0)
            | (ordered[:, -1] >= deepseek_v4_flash.NUM_ROUTED_EXPERTS)
        ).any()
    ):

...
def allocate_dep4_dspark_runtime(
    ...
    *,
    ...
    window_pages = (
        deepseek_v4_flash.DSPARK_WINDOW_TOKENS // deepseek_v4_flash.DSPARK_PAGE_TOKENS
    )
    windows = tuple(
        torch.zeros(
...
def _allocate_workspace(torch, device, groups):
    ...
    return DeepSeekV4DSparkWorkspace(
        ...
        empty(
            (
                rows,
                3 * deepseek_v4_flash.HIDDEN_SIZE // deepseek_v4_flash.FP8_BLOCK_SIZE,
            ),
            torch.float32,
        ),
        ...
        empty(
            (groups, deepseek_v4_flash.TARGET_STATE_SLOT_SHAPES.raw_window[-1]),
            torch.uint8,
        ),
        empty((rows, deepseek_v4_flash.HIDDEN_SIZE)),
        ...
        empty(
            (rows, deepseek_v4_flash.HIDDEN_SIZE + deepseek_v4_flash.DSPARK_MARKOV_RANK)
        ),

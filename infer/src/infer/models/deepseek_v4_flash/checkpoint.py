...
_ROOT_TARGET_SPECS = {
    ...
    for spec in (
        ...
        deepseek_v4_flash.WeightSpec(
            "hc_head_base", (deepseek_v4_flash.MHC_STREAMS,), "F32"
        ),
        deepseek_v4_flash.WeightSpec(
            "hc_head_fn",
            (
        ...
        deepseek_v4_flash.WeightSpec(
            "norm.weight", (deepseek_v4_flash.HIDDEN_SIZE,), "BF16"
        ),
    )
}
_BLOCK_NAMES = (
...
_ROUTED_EXPERT_SPECS = {
    spec.name: spec for spec in deepseek_v4_flash.ROUTED_EXPERT_TEMPLATE
}
...
def _expected_dspark_weight_map() -> dict[str, str]:
    ...
    for stage_id in range(deepseek_v4_flash.DSPARK_STAGE_COUNT):
        ...
        if stage_id == deepseek_v4_flash.DSPARK_STAGE_COUNT - 1:
            names.update(
                spec.name for spec in deepseek_v4_flash.DSPARK_ENDPOINT_WEIGHTS
            )
        weight_map.update(
            dict.fromkeys(

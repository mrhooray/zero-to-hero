...
_RAW_RING_PAGES = (
    deepseek_v4_flash.RAW_STATE_RING_TOKENS // deepseek_v4_flash.DSPARK_PAGE_TOKENS
)
...
_C128_STATE_PAGES = (
    _C128_STATE_TABLE_WIDTH + 16 * deepseek_v4_flash.MAX_PREFILL_REQUESTS
)
_C128_SELECTED_WIDTH = (
    (
        deepseek_v4_flash.MAX_CONTEXT_TOKENS // 128
        + deepseek_v4_flash.SWA_WINDOW_TOKENS
        + 127
    )
    // 128
    * 128
    ...
    *,
    ...
    history = tuple(
        ...
        for layer_ids, layout in (
            (
                deepseek_v4_flash.CSA_LAYER_IDS,
                deepseek_v4_flash.C4_FP8_MAIN_HISTORY_CANDIDATE,
            ),
            (
                deepseek_v4_flash.CSA_LAYER_IDS,
                deepseek_v4_flash.C4_FP4_INDEX_HISTORY_CANDIDATE,
            ),
            (
                deepseek_v4_flash.HCA_LAYER_IDS,
                deepseek_v4_flash.C128_FP8_MAIN_HISTORY_CANDIDATE,
            ),
        )
    )
    cos_sin = (
    ...
    result_records = torch.zeros(
        (
            state_slots,
            1 + (deepseek_v4_flash.DSPARK_VERIFY_WIDTH if speculative else 1),
        ),
        dtype=torch.int64,
        device=device,
    )
    ...
    for groups in (0, *deepseek_v4_flash.DECODE_BATCH_SIZES):
        batch_size = (
            groups * deepseek_v4_flash.DSPARK_VERIFY_WIDTH if speculative else groups
        )
...
def _allocate_prefill_resources(
    ...
    attention_rows = (
        ...
        * (
            deepseek_v4_flash.INDEX_CONTEXT_CAPACITY
            + deepseek_v4_flash.SWA_WINDOW_TOKENS
        )
        + tokens
    )
...
def _prefill_layer_views(runtime, batch, layer_id):
    ...
    selected_width = (
        ...
        if ratio == 4
        else (batch.table_width if ratio == 128 else 0)
        + deepseek_v4_flash.SWA_WINDOW_TOKENS
    )
    ...
    rows = (
        requests * (compressed_capacity + deepseek_v4_flash.SWA_WINDOW_TOKENS) + tokens
    )
    ...
    def verify(self, runtime: DeepSeekV4VerifyRuntime[object]) -> None:
        ...
        logits = self._decode(
            ...
            runtime.captured_hidden.view(
                -1,
                len(deepseek_v4_flash.DSPARK_TARGET_LAYER_IDS)
                * deepseek_v4_flash.HIDDEN_SIZE,
            ),
        )

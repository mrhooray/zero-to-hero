...
class IncrementalDecoder:
    ...
    def finish(self, token_ids: Sequence[int]) -> str:
        decoded = self._tokenizer.decode(list(token_ids), skip_special_tokens=True)
        streamed = "".join(self._chunks)
        if not decoded.startswith(streamed):
            raise RuntimeError("incremental decoding diverged from full decoding")
        return decoded[len(streamed) :]

...
def build_workload(
    ...
    *,
    ...
    precomputed = {int(group["group"]): _request_digests(group) for group in groups}
    requests = [
        build_request(spec, groups[int(spec["group"])], precomputed[int(spec["group"])])
    ...
    for _ in range(12):
        ...
        if len(token_ids) == PROMPT_TOKENS:
            return {
                ...
                "rendered_prefix_sha256": digest_json(token_ids[:SHARED_PREFIX_TOKENS]),
...
def validate_workload(workload: dict[str, object], codec=None) -> dict[str, object]:
    ...
    for request, expected in zip(requests, expected_schedule, strict=True):
        ...
        expected_prompt = _memo(
            digest_cache, prompt_key, lambda token_ids=token_ids: digest_json(token_ids)
        )
        ...
        expected_messages = _memo(
            digest_cache, message_key, lambda messages=messages: digest_json(messages)
        )
        ...
        if codec is not None:
            ...
            rendered = _memo(
                render_cache, render_key, lambda content=content: encode(codec, content)
            )
...
def run_decode_benchmark(
    ...
    receipt["qualification"] = priming_qualification | {
        ...
        "speculation": config["resolved_server_config"].get("speculation"),
...
def execute_request(
    ...
    *,
    ...
    response = request_stream(
        ...
        origin=origin,
        expected_cached=expected_cached,
        routed_dp_rank=int(spec["group"]) if config["engine"] == "sglang" else None,
    )
    keys = (
        "ordinal",
        "phase",
...
def request_stream(
    ...
    *,
    ...
    last = float(parsed.pop("last_token_at"))
    return {
        **parsed,
...
def request_body(
    *,
    ...
    if routed_dp_rank is not None:
        if not 0 <= routed_dp_rank < GROUP_COUNT:
            raise ValueError("routed DP rank is out of range")

...
run anywhere; Triton/SM100 kernels need B200):
    ...
    uv run modal run -e <env> tools/modal_pytest.py --selector tests/gpu/test_glm53_distributed_argmax.py
...
app = modal.App(secrets.token_hex(8))
...
def _run(selector: str) -> int:
    ...
    digest = hashlib.sha256(
        "".join(
            sorted(
                f"{path.name}:{path.stat().st_size}" for path in root.glob("test_*.py")
            )
        ).encode()
    ).hexdigest()[:12]
    print(f"gpu-suite: {digest}", flush=True)
    subprocess.run(["nvidia-smi", "-L"], cwd="/repo", check=False)

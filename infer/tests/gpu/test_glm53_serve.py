...
class GLM53ServeTest(unittest.TestCase):
    def test_rank_zero_warms_dep4_buckets_and_builds_four_lane_scheduler(
        ...
        with (
            ...
            patch.object(serve, "require_memory_reserve") as require_memory_reserve,
        ...
        self.assertEqual(nextn_runtime.seed_candidates.call_count, 2 * len(buckets))
        ...
        self.assertEqual(
            ...
            {
                "token_budget": (glm53_flash.KDA_CHUNK_SIZE + max(buckets) * width),
    ...
    def test_target_only_build_skips_nextn_and_uses_single_token_decode(
        ...
        with (
            ...
            patch.object(
                serve, "Scheduler", return_value="scheduler"
            ) as make_scheduler,
            patch.object(serve, "DistributedEngine", return_value="engine"),
    ...
    def test_rank_startup_gates_serving_on_all_rank_readiness(self) -> None:
        ...
        with (
            ...
            patch.object(
                serve,
                "_create_all_reduce",
                side_effect=lambda *_: events.append("all_reduce_create") or all_reduce,
            ),
            patch.object(
                serve,
                "_build_engine",

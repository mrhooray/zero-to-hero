...
from infer.models.deepseek_v4_flash import attention, checkpoint, megamoe, target
from infer.models.deepseek_v4_flash import model as deepseek_v4

OPS_SOURCE = (
    Path(__file__).resolve().parents[2]
    / "src/infer/models/deepseek_v4_flash/ops/target.py"
).read_text()
...
class DeepSeekV4TargetWeightsTest(unittest.TestCase):
    ...
    def test_prefill_stages_packed_rows_in_both_descriptor_parities(self) -> None:
        ...
        with patch.dict(
            sys.modules, {"infer.models.deepseek_v4_flash.ops.stage": stage}
        ):
            for parity in (0, 1):
    ...
    def test_eager_prefill_runs_all_layer_kinds_and_sampled_head_rows(self) -> None:
        ...
        self.assertEqual(
            [call[2] for call in calls].count("swa"),
            sum(
                1
                for layer_id in range(deepseek_v4.NUM_LAYERS)
                if deepseek_v4.attention_kind(layer_id) == "swa"
            ),
        )
        self.assertEqual(

...
class GLM53NextNResidentRuntimeTest(unittest.TestCase):
    ...
    def test_packed_catchup_shifts_fresh_and_resumed_sequences(self) -> None:
        ...
        runtime = nextn.GLM53NextNRuntime(
            ...
            prefill_token_ids=torch.empty(
                glm53_flash.KDA_CHUNK_SIZE, dtype=torch.int64
            ),
            draft_hidden=torch.zeros(
                (4, glm53_flash.HIDDEN_SIZE), dtype=torch.bfloat16
            ),
            ...
            with self.subTest(accepted=accepted):
                self.assertEqual(
                    _accept_glm53_greedy_chain(
                        tuple(candidates), targets, VERIFY_WIDTH
                    ),
                    targets[:accepted],
                )
...
class GLM53NextNCheckpointTest(unittest.TestCase):
    def test_layer45_contract_uses_two_pinned_shards(self) -> None:
        ...
        primary, secondary = (shard[0] for shard in glm53_checkpoint.NEXTN_LAYER_SHARDS)

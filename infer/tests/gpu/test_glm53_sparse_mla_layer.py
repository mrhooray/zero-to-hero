...
def load_ops_module():
    ...
    sparse_prefill = types.ModuleType("infer.models.glm53_flash.ops.sparse_mla_prefill")
...
class SparseMLALayerLoaderTest(unittest.TestCase):
    def test_all_sparse_shell_routes_match_pinned_index(self) -> None:
        self.assertEqual(
            tuple(
                field.name for field in fields(glm53_flash.GLM53SparseLayerShellWeights)
            ),
            (
                "attention_mhc",
                "input_norm",
    ...
    def test_shell_scope_ignores_attention_and_rejects_extra_shell_key(self) -> None:
        ...
        extra = f"{glm53_checkpoint.DENSE_LAYER_PREFIX.format(layer_id=layer_id)}hc_attn_extra"
    ...
    def test_full_model_dispatch_inventory_matches_pinned_index(self) -> None:
        ...
        self.assertEqual(
            glm53_checkpoint._glm53_nextn_checkpoint_keys(), frozenset(nextn)
        )

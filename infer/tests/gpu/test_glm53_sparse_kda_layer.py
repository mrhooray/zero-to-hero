...
class SparseKDALayerLoaderTest(unittest.TestCase):
    def test_composite_loader_calls_each_component_once(self) -> None:
        self.assertEqual(
            tuple(
                field.name for field in fields(glm53_flash.GLM53SparseKDALayerWeights)
            ),
            (
                "attention_mhc",
                "input_norm",
...
class SparseKDALayerAliasTest(unittest.TestCase):
    ...
    def fixture(self):
        ...
        attention = glm53_flash.KDAWeights(
            *(self.tensor() for _ in fields(glm53_flash.KDAWeights))
        )

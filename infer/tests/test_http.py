...
def completed_submission(prompt_tokens: int = 2, cached_tokens: int = 0) -> Submission:
...
class HTTPTest(unittest.IsolatedAsyncioTestCase):
    def setUp(self) -> None:
        ...
        self.codec.incremental_decoder.return_value.finish.return_value = ""
    ...
    async def test_stream_flushes_text_buffered_by_incremental_decoding(self) -> None:
        def submit(
            _prompt_token_ids: tuple[int, ...],
            _max_output_tokens: int,
            *,
        ...
        self.service.submit.side_effect = submit
        decoder = self.codec.incremental_decoder.return_value
        decoder.decode.return_value = "hel"
        decoder.finish.return_value = "lo"
        ...
        decoder.finish.assert_called_once_with((20, 21))

...
class IncrementalDecoderTest(unittest.TestCase):
    ...
    def test_finishes_with_text_buffered_by_the_stream_decoder(self) -> None:
        tokenizer = Mock(spec=Tokenizer)
        tokenizer.decode.return_value = "hello"
        stream = Mock()
        stream.step.side_effect = ("hel", None)

        with patch("infer.codec.DecodeStream", return_value=stream):
            decoder = IncrementalDecoder(tokenizer)
            self.assertEqual(decoder.decode((10, 11)), "hel")
            self.assertEqual(decoder.finish((10, 11)), "lo")

        tokenizer.decode.assert_called_once_with([10, 11], skip_special_tokens=True)
